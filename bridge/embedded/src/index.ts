...
function launchNapukettoBranch(
    ...
    if (decision.action === "skip") {
        logger.error(`napuketto 未拉起：${decision.reason}`);
        return null;
    }
...
async function main(): Promise<void> {
    ...
    const relay = new Relay({
        ...
        configStore,
        onShutdown: (reason) => {
            logger.info(`收到 Java 关机通知（${reason}），退出`);
            void shutdown(0);
        },
    });
    ...
    if (napukettoEnabled) {
        ...
        stub.on("exit", (code) => {
            logger.info(`stub 协议端退出（code=${code ?? "null"}）`);
        });
        logger.info(`stub 协议端已拉起：${stubPath}`);
    } else {
        logger.info("未配置 KUROBRIDGE_STUB_PEER，跳过 stub 拉起（external 形态）");
    }
...
main().catch((error: unknown) => {
    // main 失败可能早于 logger 实例创建：此处新建（行格式契约仍收敛在 logger.ts，ADR-034）
