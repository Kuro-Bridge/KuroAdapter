...
- 工作流：`pnpm check`（biome + tsc，提交前必跑）、`pnpm fix`（自动修复）、`pnpm test`（vitest）、`pnpm -r build`（TS 全量构建）、`gradlew build`（Java 侧，platforms/je）、`pnpm build:jar`（全链路：TS 构建 → 嵌入式打包（toolings/packaging/embed.ts）→ gradle :paper:shadowJar，经 toolings/packaging/build-jar.mjs 链式执行）。
